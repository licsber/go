...
import (
	"encoding/hex"
	"github.com/licsber/go/lHash/md4"
	"runtime"
)

// Size The size of the ed2k checksum in bytes.
const Size = md4.Size

// BlockSize The size of each hash node in bytes.
...
func (d *digest) hashLoop() {
	...
	for {
		...
		if l := len(runningHashes); l > 0 {
			// make Write() block if we already have 2 * GO MAX PROC hashes flying
		...
		case bytes := <-nextHash:
			hashList = append(hashList, bytes...)
...
// Close Stops the background HASHer and releases all memory
// used by chunks.
//
